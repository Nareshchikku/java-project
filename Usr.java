...
public class Usr extends Admin{
		...
		public boolean checkexist() throws SQLException {
		 ...
		 public boolean insert()throws SQLException {
			...
			Connection conn=Reg.Establish();
			 Statement st=conn.createStatement();
			 String sql="call newreg('"+udid+"','"+pwd+"','"+hno+"','"+cid+"','"+age+"','"+gender+"','"+phn_no+"','"+fname+"','"+dob+"','"+sname+"','"+sno+"','"+locality+"','"+pincode+"','"+cname+"','"+cage+"','"+cgender+"','"+cphno+"','"+chno+"','"+cstreet+"','"+cstno+"','"+cloc+"','"+cpin+"','"+tod+"')";
		 ...
		 public static <T> ArrayList<String> schemes(String a) throws SQLException,NumberFormatException{
				...
				ResultSet rs=st.executeQuery("select ds.schme,ds.desp from display_schemes as ds,typeofdisability_details as td where td.tod=ds.tod and td.udid='"+a+"'");
				while(rs.next()) {
					b.add(rs.getString("schme"));
					b.add(rs.getString("desp"));
				}
				Reg.closeConnection(conn);
		 ...
		 public String change(String p,String l,String u_name) {
			 try
			 {
			 Connection conn=Reg.Establish();
			 Statement st=conn.createStatement();
			 st = conn.createStatement(); 
			 ResultSet rs;
			 rs=st.executeQuery("select *from login where udid='"+u_name+"'");
			 if(rs.next())
			 {
			 	if(rs.getString("pwd").equals(p)) {
			 		st.executeUpdate("update login set pwd='"+l+"' where udid='"+u_name+"'");
			 		Reg.closeConnection(conn);
